...
export const webEvidenceSearchTool = tool(
  async ({ url, goal }: { url: string; goal: string }) => {
    if (!browserAutomationEnabled()) {
      return "agent-browser is disabled in this deployment (set ENABLE_AGENT_BROWSER=true and provision Chrome for Testing to enable live browsing). Reason this over general knowledge instead.";
    }
    try {
      ...
      return `agent-browser navigation failed for ${url}: ${message}. Fall back to general medical/fitness knowledge and say the source could not be verified live.`;
    }
  },
  {
